...
const searchVideos = async (q) => {
   try {
      const resService = await service.search.list({
         q: q,
         part: ["snippet"],
         maxResults: 10,
         channelType: "any",
      ...
      for (let i = 0; i < videos.items.length; i++) {
         const resChannelService = await channelService.getById(videos.items[i].snippet.channelId)
...
const getByVideoCategoryId = async (categoryId) => {
   try {
      ...
      for (let i = 0; i < videos.length; i++) {
         const resChannelService = await channelService.getById(videos[i].snippet.channelId)
         const channels = resChannelService.data.items;
